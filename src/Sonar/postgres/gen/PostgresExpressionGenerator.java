...
import Sonar.postgres.ast.PostgresBinaryRangeOperation.PostgresBinaryRangeComparisonOperator;
import Sonar.postgres.ast.PostgresBinaryRangeOperation.PostgresBinaryRangeOperator;
import Sonar.postgres.ast.PostgresFunction.PostgresFunctionWithResult;
import Sonar.postgres.ast.PostgresOrderByTerm.PostgresOrder;
import Sonar.postgres.ast.PostgresPOSIXRegularExpression.POSIXRegex;
...
public class PostgresExpressionGenerator implements ExpressionGenerator<PostgresExpression> {
    ...
    private enum ColumnExpressionActions {
        BINARY_COMPARISON, BINARYArithmetic


    }
    private enum  WhereColumnExpression {
        BINARYArithmetic,UNARY_POSTFIX
    }

    public PostgresExpression generateFetchColumnExpression(PostgresColumn c) {
        PostgresDataType ctype = c.getType();
        switch (Randomly.fromOptions(PostgresExpressionGenerator.ColumnExpressionActions.values())) {
            case BINARY_COMPARISON:
                return new PostgresBinaryComparisonOperation(new PostgresColumnReference(c), generateExpression(2,ctype),
            ...
            case BINARYArithmetic:
                return new PostgresBinaryArithmeticOperation(
                        new PostgresColumnReference(c),
                        generateExpression(2,ctype),
                        PostgresBinaryArithmeticOperation.PostgresBinaryOperator.getRandom());
            default:
                throw new AssertionError();
        }
    }

    public PostgresExpression generateWhereColumnExpression(PostgresPostfixText postfixText, PostgresColumn c) {
        ...
        PostgresDataType ctype = c.getType();
        switch (Randomly.fromOptions(PostgresExpressionGenerator.WhereColumnExpression.values())) {
            case BINARYArithmetic:
                return new PostgresBinaryArithmeticOperation(new PostgresManuelPredicate(postfixText.getText()), generateConstant(new Randomly(),ctype),
                        PostgresBinaryArithmeticOperation.PostgresBinaryOperator.getRandom());
            case UNARY_POSTFIX:
                return new PostgresManuelPredicate(postfixText.getText());
            default:
                throw new AssertionError();
        }
    }
