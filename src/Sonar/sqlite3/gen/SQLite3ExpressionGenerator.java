...
import Sonar.sqlite3.ast.SQLite3Constant.SQLite3TextConstant;
...
import Sonar.sqlite3.ast.SQLite3Expression.BinaryComparisonOperation.BinaryComparisonOperator;
...
import Sonar.sqlite3.ast.SQLite3Expression.SQLite3OrderingTerm.Ordering;
...
import Sonar.sqlite3.ast.SQLite3Expression.Sqlite3BinaryOperation.BinaryOperator;
import Sonar.sqlite3.ast.SQLite3Expression.TypeLiteral;
import Sonar.sqlite3.ast.SQLite3Function;
import Sonar.sqlite3.ast.SQLite3Function.ComputableFunction;
...
import Sonar.sqlite3.schema.SQLite3Schema.SQLite3Column.SQLite3CollateSequence;
...
public class SQLite3ExpressionGenerator implements ExpressionGenerator<SQLite3Expression> {
    ...
    enum ColumnExprType {
        BINARY_OPERATOR, BINARY_COMPARISON_OPERATOR, Function,
        DateFunction,


    }

    enum WhereColumnExpression {
        BINARY_COMPARISON_OPERATION, BINARY_OPERATION, UNARY_POSTFIX
    }

    public SQLite3Expression generateFetchColumnExp(SQLite3Column column) {

        switch (Randomly.fromOptions(ColumnExprType.values())) {
            case BINARY_OPERATOR:
                return new SQLite3Expression.Sqlite3BinaryOperation(new SQLite3ColumnName(new SQLite3Column(column.getName(), SQLite3DataType.INT, false, false, null), null),
                        getRandomExpression(2), BinaryOperator.getRandomOperator());
            case BINARY_COMPARISON_OPERATOR:
                return new SQLite3Expression.BinaryComparisonOperation(new SQLite3ColumnName(new SQLite3Column(column.getName(), SQLite3DataType.INT, false, false, null), null),
                        getRandomExpression(2), BinaryComparisonOperator.getRandomOperator());
            case Function:
                return getFunction(globalState, 2);
            case DateFunction:
                return new SQLite3Expression.Sqlite3BinaryOperation(new SQLite3ColumnName(new SQLite3Column(column.getName(), SQLite3DataType.INT, false, false, null), null),
                        new SQLite3Expression.SQLite3ManuelPredicate(generateDateFunc()), BinaryOperator.getRandomOperator());
            default:
    ...
    public SQLite3Expression generateWhereColumnExpression(SQLite3Expression.SQLite3ManuelPredicate postfixText) {
        switch (Randomly.fromOptions(WhereColumnExpression.values())) {
            case BINARY_COMPARISON_OPERATION:
                return new SQLite3Expression.Sqlite3BinaryOperation(new SQLite3Expression.SQLite3ManuelPredicate(postfixText.getString()), getRandomLiteralValue(globalState),
                        BinaryOperator.getRandomOperator());
            case BINARY_OPERATION:
                return new SQLite3Expression.BinaryComparisonOperation(new SQLite3Expression.SQLite3ManuelPredicate(postfixText.getString()), getRandomLiteralValue(globalState),
                        BinaryComparisonOperator.getRandomOperator());
            case UNARY_POSTFIX:
                return new SQLite3Expression.SQLite3ManuelPredicate(postfixText.getString());
    ...
    public String generateDateFunc() {
        Random random = new Random();
        String[] functions = {"date", "datetime", "julianday", "unixepoch"};
        String[] modifiers = {"+1 day", "-1 day", "+1 hour", "-1 hour", "+1 minute", "-1 minute", "+1 second", "-1 second"};
        ...
        String date = "'2019-08-31'";
        String datetime = "'2019-08-31 02:34:56'";


        String expression;
        switch (function) {
            case "date":
                expression = function + "(" + date + "," + modifier + ")";
                break;
            case "datetime":
                expression = function + "(" + datetime + "," + modifier + ")";
                break;
            case "julianday":
                expression = function + "(" + date + "," + modifier + ")";
                break;
